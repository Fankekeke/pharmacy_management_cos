<template>
  <a-modal v-model="show" title="修改药品记录" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='药品现名' v-bind="formItemLayout">
            <a-input v-decorator="[
            'drugName',
            { rules: [{ required: true, message: '请输入药品现名!' }] }
            ]"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label='曾用名' v-bind="formItemLayout">
            <a-input v-decorator="[
            'formerName',
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...drug}) {
      this.rowId = drug.id
      let fields = ['drugName', 'formerName', 'content', 'images', 'shelvesAddress', 'stockAddress']
      let obj = {}
